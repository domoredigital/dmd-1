...
// Build a short plain-text recap (~400 chars max) from the most recent
// session(s) so a returning user can be greeted with continuity. No AI — just
// the last several user-message topics condensed into one line.
export function loadRecentContext() {
  const sessions = loadSessions();
  if (!sessions.length) return '';

  // Walk sessions newest-first, collecting recent user messages.
  const userLines = [];
  for (let i = sessions.length - 1; i >= 0 && userLines.length < 6; i--) {
    const msgs = sessions[i].messages || [];
    for (let j = msgs.length - 1; j >= 0 && userLines.length < 6; j--) {
      if (msgs[j].role === 'user' && msgs[j].content) {
        userLines.push(String(msgs[j].content).trim());
      }
    }
  }
  if (!userLines.length) return '';

  // Oldest-to-newest reads more naturally, then trim each to a short phrase.
  const topics = userLines
    .reverse()
    .map((line) => (line.length > 70 ? line.slice(0, 67).trim() + '…' : line));

  let recap = `Recently talked about: ${topics.join('; ')}.`;
  if (recap.length > 400) recap = recap.slice(0, 397).trim() + '…';
  return recap;
}
